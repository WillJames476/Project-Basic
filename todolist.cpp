 ...
 * date last updated : November 5 2021
 * 
 * version 1.0:
...
void add_task_incrementally(const std::string &task_name, std::tm time_start, Task_list &task_list, int day_increment)
{
    ...
    {
        for(;time_start.tm_mday <= 31;time_start.tm_mday+= day_increment)
        {
...
void get_a_date_from_user(std::tm& to_return)
{
    ...
    to_return.tm_mday = get_a_time_from_user("day", current_date->tm_mday,33);
}
